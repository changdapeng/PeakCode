...
import { DEFAULT_GIT_TEXT_GENERATION_MODEL } from "@peakcode/contracts";
import { sanitizeGeneratedThreadTitle } from "@peakcode/shared/chatThreads";
import { resolveCodexHome } from "@peakcode/shared/codexConfig";
...
const makeCodexTextGeneration = Effect.gen(function* () {
  ...
  const writeTempFile = (
    ...
    const filePath = path.join(tempDir, `peakcode-${prefix}-${process.pid}-${randomUUID()}.tmp`);
  ...
  const prepareIsolatedCodexHome = (
    ...
    Effect.gen(function* () {
      ...
      const isolatedHomePath = path.join(
        tempDir,
        `peakcode-codex-home-${process.pid}-${randomUUID()}`,
      );
