...
const CodexTextGenerationTestLayer = CodexTextGenerationLive.pipe(
  Layer.provideMerge(
    ServerConfig.layerTest(process.cwd(), {
      prefix: "peakcode-codex-text-generation-test-",
    }),
  ),
...
function withFakeCodexEnv<A, E, R>(
  ...
  return Effect.acquireUseRelease(
    Effect.gen(function* () {
      ...
      const fs = yield* FileSystem.FileSystem;
      const tempDir = yield* fs.makeTempDirectoryScoped({ prefix: "peakcode-codex-text-" });
      ...
      const previousDisableBrowserPlugin =
        process.env.PEAKCODE_DISABLE_CODEX_PEAKCODE_BROWSER_PLUGIN;
    ...
    withFakeCodexEnv(
      ...
      Effect.gen(function* () {
        ...
        const wrongCodexHome = yield* fs.makeTempDirectoryScoped({
          prefix: "peakcode-wrong-codex-",
        });
        const customCodexHome = yield* fs.makeTempDirectoryScoped({
          prefix: "peakcode-custom-codex-",
        });
