...
export const PEAKCODE_CODEX_HOME_OVERLAY_DIR = "codex-home-overlay";
export const PEAKCODE_DISABLE_CODEX_PEAKCODE_BROWSER_PLUGIN_ENV =
  "PEAKCODE_DISABLE_CODEX_PEAKCODE_BROWSER_PLUGIN";
...
export function shouldDisablePeakCodeBrowserPlugin(env: NodeJS.ProcessEnv): boolean {
  // The plugin is disabled by default; the only way to opt out is the explicit "0" sentinel.
  return env[PEAKCODE_DISABLE_CODEX_PEAKCODE_BROWSER_PLUGIN_ENV] !== "0";
}

export function resolvePeakCodeCodexHomeOverlayPath(
  env: NodeJS.ProcessEnv,
  sourceHomePath: string,
): string {
  const runtimeHome = env.PEAKCODE_HOME?.trim() || env.PEAKCODE_HOME_LEGACY?.trim();
  const overlayRoot =
    runtimeHome || path.join(path.dirname(sourceHomePath), ".peakcode", "runtime");
  return path.join(overlayRoot, PEAKCODE_CODEX_HOME_OVERLAY_DIR);
}

/**
 * Returns the home directory that the codex app-server child process actually
 * writes under. This is the overlay home when Peak Code wraps Codex with the
 * peakcode-browser plugin disabled (the production default), otherwise the
...
export function resolveActiveCodexHomeWritePath(input: CodexHomePathsInput = {}): string {
  ...
  if (!shouldDisablePeakCodeBrowserPlugin(env)) {
    return source;
  }
  const overlay = resolvePeakCodeCodexHomeOverlayPath(env, source);
