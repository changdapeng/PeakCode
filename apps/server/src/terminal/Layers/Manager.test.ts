...
describe("TerminalManager", () => {
  ...
  function makeManager(
    ...
    const logsDir = fs.mkdtempSync(path.join(os.tmpdir(), "peakcode-terminal-"));
  ...
  it("filters app runtime env variables from terminal sessions", async () => {
    ...
    try {
      ...
      expect(spawnInput.env.PEAKCODE_PORT).toBeUndefined();
  ...
  it("injects runtime env overrides into spawned terminals", async () => {
    ...
    expect(spawnInput.env.PEAKCODE_PROJECT_ROOT).toBe("/repo");
    expect(spawnInput.env.PEAKCODE_WORKTREE_PATH).toBe("/repo/worktree-a");
