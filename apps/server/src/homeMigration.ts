 ...
 * Purpose: Imports legacy ~/.t3 state into the new ~/.peakcode home on first startup.
...
export const PEAKCODE_HOME_DIRNAME = ".peakcode";
...
const snapshotSqliteDatabase = (sourcePath: string, targetPath: string) =>
  Effect.tryPromise({
    try: async () => {
      ...
      new HomeMigrationError({
        message: `Failed to snapshot legacy sqlite database from ${sourcePath} to ${targetPath}. Close other Peak Code processes and retry.`,
        cause,
      }),
  });
...
export const migrateLegacyHomeIfNeeded = Effect.fn(function* (input: LegacyHomeMigrationInput) {
  const fs = yield* FileSystem.FileSystem;
  const path = yield* Path.Path;
  const canonicalTargetBaseDir = path.resolve(path.join(input.homeDir, PEAKCODE_HOME_DIRNAME));
  ...
  const stagingBaseDir = path.join(
    input.homeDir,
    `.${PEAKCODE_HOME_DIRNAME.slice(1)}-migration-${process.pid}-${Date.now()}`,
  );
  ...
  const migrateEffect = Effect.gen(function* () {
    ...
    yield* writeMigrationMarker(markerPath, {
      ...
      notes: [
        "Legacy ~/.t3 data is being imported into ~/.peakcode.",
    ...
    yield* writeMigrationMarker(markerPath, {
      ...
      notes: [
        "Legacy ~/.t3 data was imported into ~/.peakcode.",
        "Existing legacy worktree directories were left in place and are still referenced by absolute path.",
      ],
    });

    yield* Effect.logInfo("imported legacy T3 state into Peak Code home", {
      sourceStateDir: sourcePaths.stateDir,
      targetStateDir: targetPaths.stateDir,
      importedArtifacts,
    });
  ...
  return yield* migrateEffect.pipe(
    ...
    Effect.mapError((error) =>
      error instanceof HomeMigrationError
        ? error
        : new HomeMigrationError({
            message: "Failed to import legacy ~/.t3 state into ~/.peakcode.",
            cause: error,
          }),
    ),
  );
