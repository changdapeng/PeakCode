 ...
 * Purpose: Verifies first-run import and resume behavior for the ~/.t3 -> ~/.peakcode migration.
...
it.layer(NodeServices.layer)("homeMigration", (it) => {
  it.effect("imports legacy userdata into the new default home", () =>
    Effect.gen(function* () {
      ...
      const tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "peakcode-home-migration-"));
      ...
      const targetBaseDir = path.join(tempHome, PEAKCODE_HOME_DIRNAME);
