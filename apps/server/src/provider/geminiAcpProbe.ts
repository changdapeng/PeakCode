...
function formatGeminiDiscoveryWarning(detail: string): string {
  return `Gemini CLI is installed, but Peak Code could not verify authentication or discover models. ${detail}`;
}
...
function formatGeminiModelDiscoveryFallbackMessage(): string {
  return "Gemini CLI is installed and authenticated, but it did not report any available models. Peak Code will use its built-in Gemini model list.";
}
