...
const PEAKCODE_GEMINI_SETTINGS_DIR = path.join(os.tmpdir(), "peakcode", "gemini");
...
const makeGeminiAdapter = Effect.fn("makeGeminiAdapter")(function* (
  ...
  const prepareGeminiLaunchConfig = Effect.fn("prepareGeminiLaunchConfig")(function* (input: {
    ...
    const systemSettingsPath = path.join(
      PEAKCODE_GEMINI_SETTINGS_DIR,
      `${input.threadId}-${crypto.randomUUID()}.json`,
    );
    yield* Effect.tryPromise({
      try: async () => {
        await fs.mkdir(PEAKCODE_GEMINI_SETTINGS_DIR, { recursive: true });
        await fs.writeFile(
          systemSettingsPath,
          JSON.stringify(
            {
