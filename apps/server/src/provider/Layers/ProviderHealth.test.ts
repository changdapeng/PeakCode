...
import { PEAKCODE_CODEX_HOME_OVERLAY_DIR } from "../../codexHomePaths";
import {
  ...
  return Effect.gen(function* () {
    ...
    yield* Effect.acquireRelease(
      Effect.sync(() => {
        ...
        const originalPeakcodeHome = process.env.PEAKCODE_HOME;
        const originalPortkeyApiKey = process.env.PORTKEY_API_KEY;
        process.env.CODEX_HOME = tmpDir;
        process.env.PEAKCODE_HOME = runtimeDir;
        process.env.PORTKEY_API_KEY ??= "test-portkey-key";
        return { originalCodexHome, originalPeakcodeHome, originalPortkeyApiKey };
      }),
      ({ originalCodexHome, originalPeakcodeHome, originalPortkeyApiKey }) =>
        Effect.sync(() => {
          ...
          if (originalPeakcodeHome !== undefined) {
            process.env.PEAKCODE_HOME = originalPeakcodeHome;
          } else {
  ...
  describe("checkCodexProviderStatus", () => {
    ...
    it.effect("uses configured codex home for version, config, and auth probes", () => {
      ...
      return Effect.gen(function* () {
        ...
        expectedCodexHome = path.join(runtimeDir, PEAKCODE_CODEX_HOME_OVERLAY_DIR);
