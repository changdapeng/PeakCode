...
describe("resolveCodexGeneratedImagesRoot(s)", () => {
  ...
  const previousLegacyHome = process.env.PEAKCODE_HOME_LEGACY;
  const previousDisableFlag = process.env.PEAKCODE_DISABLE_CODEX_PEAKCODE_BROWSER_PLUGIN;

  afterEach(() => {
    if (previousPeakcodeHome === undefined) delete process.env.PEAKCODE_HOME;
    else process.env.PEAKCODE_HOME = previousPeakcodeHome;
    if (previousLegacyHome === undefined) delete process.env.PEAKCODE_HOME_LEGACY;
    else process.env.PEAKCODE_HOME_LEGACY = previousLegacyHome;
    if (previousDisableFlag === undefined)
      delete process.env.PEAKCODE_DISABLE_CODEX_PEAKCODE_BROWSER_PLUGIN;
    else process.env.PEAKCODE_DISABLE_CODEX_PEAKCODE_BROWSER_PLUGIN = previousDisableFlag;
  });

  it("returns the overlay generated_images directory as the active write root by default", () => {
    process.env.PEAKCODE_HOME = "/peakcode-test/runtime";
    delete process.env.PEAKCODE_DISABLE_CODEX_PEAKCODE_BROWSER_PLUGIN;
    assert.equal(
      resolveCodexGeneratedImagesRoot("/codex-test/.codex"),
      path.join("/peakcode-test/runtime", "codex-home-overlay", "generated_images"),
    );
  });

  it("returns the source generated_images directory when the peakcode-browser plugin is enabled", () => {
    process.env.PEAKCODE_HOME = "/peakcode-test/runtime";
    process.env.PEAKCODE_DISABLE_CODEX_PEAKCODE_BROWSER_PLUGIN = "0";
    assert.equal(
      resolveCodexGeneratedImagesRoot("/codex-test/.codex"),
  ...
  it("collapses to a single root when overlay equals source", () => {
    ...
    delete process.env.PEAKCODE_HOME_LEGACY;
    // The overlay falls under `<dirname(source)>/.peakcode/runtime/codex-home-overlay`,
    ...
    const homePath = "/runtime/.peakcode/runtime/codex-home-overlay";
