...
// Purpose: Resolves and seeds Electron userData profile paths during the Peak Code rename.
...
const DEV_USER_DATA_DIR_NAME = "peakcode-dev";
const PROD_USER_DATA_DIR_NAME = "peakcode";
...
export function seedDesktopUserDataProfileFromLegacy(input: {
  ...
  try {
    ...
    FS.writeFileSync(
      Path.join(input.targetPath, "peakcode-profile-seed.json"),
      `${JSON.stringify(
        {
          sourcePath,
