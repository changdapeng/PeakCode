...
it.layer(NodeServices.layer)("dev-runner", (it) => {
  describe("resolveOffset", () => {
    it.effect("uses explicit PEAKCODE_PORT_OFFSET when provided", () =>
      Effect.sync(() => {
        const result = resolveOffset({ portOffset: 12, devInstance: undefined });
        assert.deepStrictEqual(result, {
          offset: 12,
          source: "PEAKCODE_PORT_OFFSET=12",
        });
      }),
    );
    ...
    it.effect("throws for negative port offset", () =>
      Effect.gen(function* () {
        ...
        assert.ok(error.includes("Invalid PEAKCODE_PORT_OFFSET"));
      }),
    );
  });

  describe("createDevRunnerEnv", () => {
    it.effect("defaults PEAKCODE_HOME to ~/.peakcode when not provided", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.PEAKCODE_HOME, resolve(homedir(), ".peakcode"));
      }),
    );
    ...
    it.effect("supports explicit typed overrides", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.PEAKCODE_HOME, resolve("/tmp/custom-t3"));
        assert.equal(env.PEAKCODE_PORT, "4222");
        assert.equal(env.VITE_WS_URL, "ws://[::1]:4222");
        assert.equal(env.PEAKCODE_NO_BROWSER, "1");
        assert.equal(env.PEAKCODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD, "0");
        assert.equal(env.PEAKCODE_LOG_WS_EVENTS, "1");
        assert.equal(env.PEAKCODE_HOST, "0.0.0.0");
    ...
    it.effect("does not force websocket logging on in dev mode when unset", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev",
          baseEnv: {
            PEAKCODE_LOG_WS_EVENTS: "keep-me-out",
          },
          serverOffset: 0,
          webOffset: 0,
          t3Home: undefined,
        ...
        assert.equal(env.PEAKCODE_MODE, "web");
        assert.equal(env.PEAKCODE_LOG_WS_EVENTS, undefined);
      }),
    );
    ...
    it.effect("forwards explicit websocket logging false without coercing it away", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.PEAKCODE_LOG_WS_EVENTS, "0");
      }),
    );
    ...
    it.effect("uses custom t3Home when provided", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.PEAKCODE_HOME, resolve("/tmp/my-t3"));
      }),
    );
  });
