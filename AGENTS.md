...
- Use an isolated home dir and non-default ports when running alongside the user's own Peak Code instance, for example: `env -u T3CODE_AUTH_TOKEN T3CODE_PORT_OFFSET=3158 T3CODE_NO_BROWSER=1 bun run dev -- --home-dir ./.peakcode-pr84 --port 58090`.
- Always dry-run first when avoiding conflicts: `env -u T3CODE_AUTH_TOKEN T3CODE_PORT_OFFSET=3158 bun run dev -- --home-dir ./.peakcode-pr84 --port 58090 --dry-run`.
